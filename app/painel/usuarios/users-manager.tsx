...
export default function UsersManager({
  ...
  async function updatePassword(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!selected) return;
    const form = new FormData(event.currentTarget);
    await mutate(selected.id, { operation: 'password', password: form.get('password') }, 'Senha atualizada e sessões anteriores encerradas.');
  ...
  return (
    <main className="dashboard-page users-page">
      ...
      {showCreate && <form className="admin-create" onSubmit={createUser}>
        ...
        <div className="user-detail">
          {selected ? <>
            ...
            {mayManageSelected ? <>
              ...
              <form className="compact-action" key={`password-${selected.id}`} onSubmit={updatePassword}><label>Redefinir senha<input name="password" type="password" required minLength={8} placeholder="mínimo 8 caracteres" /></label><button disabled={busy}>Salvar senha</button></form>
...
function PersonFields({ user, allowedRoles, includeRole = false }: { user?: SafeUser; allowedRoles?: AppRole[]; includeRole?: boolean }) {
  const isPreRegistration = includeRole;
  return <div className="admin-form-grid">
    ...
    {includeRole && <label>Senha inicial <small>opcional, mínimo 8 caracteres</small><input name="password" type="password" minLength={8} autoComplete="new-password" placeholder="ex.: 12345678" /></label>}
    <label>CEP <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressPostalCode" required={!isPreRegistration} defaultValue={user?.address.postalCode} /></label><label className="wide">Logradouro <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressStreet" required={!isPreRegistration} defaultValue={user?.address.street} /></label><label>Número <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressNumber" required={!isPreRegistration} defaultValue={user?.address.number} /></label><label>Cidade <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressCity" required={!isPreRegistration} defaultValue={user?.address.city} /></label><label>Estado <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressState" required={!isPreRegistration} defaultValue={user?.address.state} /></label><label>País <small>{isPreRegistration ? 'opcional no painel' : 'obrigatório'}</small><input name="addressCountry" required={!isPreRegistration} defaultValue={user?.address.country || 'Brasil'} /></label>
    <div className="form-section-title">Dados profissionais e opcionais</div>
    ...
    <label>Lattes <small>opcional</small><input name="lattesUrl" type="url" defaultValue={user?.lattesUrl ?? ''} /></label><label>ORCID <small>opcional</small><input name="orcid" defaultValue={user?.orcid ?? ''} /></label>
    <label>Instagram <small>opcional</small><input name="instagram" type="url" defaultValue={user?.socialLinks.instagram ?? ''} /></label><label>YouTube <small>opcional</small><input name="youtube" type="url" defaultValue={user?.socialLinks.youtube ?? ''} /></label><label>LinkedIn <small>opcional</small><input name="linkedin" type="url" defaultValue={user?.socialLinks.linkedin ?? ''} /></label><label>Facebook <small>opcional</small><input name="facebook" type="url" defaultValue={user?.socialLinks.facebook ?? ''} /></label><label>X / Twitter <small>opcional</small><input name="x" type="url" defaultValue={user?.socialLinks.x ?? ''} /></label>
  </div>;
}

function personPayload(form: FormData) {
  return { fullName: form.get('fullName'), email: form.get('email'), password: form.get('password'), phone: form.get('phone'), educationLevel: form.get('educationLevel'), role: form.get('role'), professionalType: form.get('professionalType'), institutionalEmail: form.get('institutionalEmail'), functionalId: form.get('functionalId'), cpf: form.get('cpf'), addressPostalCode: form.get('addressPostalCode'), addressStreet: form.get('addressStreet'), addressNumber: form.get('addressNumber'), addressComplement: form.get('addressComplement'), addressNeighborhood: form.get('addressNeighborhood'), addressCity: form.get('addressCity'), addressState: form.get('addressState'), addressCountry: form.get('addressCountry'), lattesUrl: form.get('lattesUrl'), orcid: form.get('orcid'), socialLinks: { instagram: form.get('instagram'), youtube: form.get('youtube'), linkedin: form.get('linkedin'), facebook: form.get('facebook'), x: form.get('x') } };
}
